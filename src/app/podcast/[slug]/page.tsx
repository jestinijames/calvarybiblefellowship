...
function findPodcastByLink(link: string) {
  ...
  if (podcast) {
    ...
    return { podcastTitle, podcastDescription, podcastImage };
  } else {
    return { podcastTitle: '', podcastDescription: '', podcastImage: '' };
  }
}
...
export default function PodcastRoute({ params }: { params: { slug: string } }) {
  ...
  const { podcastTitle, podcastDescription, podcastImage } =
    findPodcastByLink(podcastLink);

  const fetchVideos = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetchData(slug, nextPageToken);
